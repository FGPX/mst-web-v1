...
function demoSpecifications(category, overview, index) {
  ...
  if (category === "dining-chair") return { diningChair: {
    ...
    maxLoadKg: null, stackable: false
  }};
...
function makeDemo(product, index) {
  ...
  return {
    ...
    dataQuality: { level: "mixed", verifiedFields: [], authorizedSourceFields: authorized, derivedFields: compact ? ["spaceProfile", "smallSpaceReason"] : ["configurable", "modular"], demoFields: demo, unknownFields: ["sku", "mpn", "gtin", "ean", "price", "availability"], lastVerifiedAt: catalog.importedAt }
  };
}
...
function applyVerified(record, enrichment) {
  ...
  if (seating) {
    ...
    if (data.functions) {
      ...
      paths.push("comfortFunctions", "specifications.seating");
    }
  ...
  if (record.specifications.bed && data.sleepingSurfaceOptionsMm) {
    ...
    paths.push("specifications.bed", "floorClearanceMm");
  }
  if (record.specifications.wardrobe && data.doorTypes) {
    ...
    paths.push("specifications.wardrobe");
  }
  if (record.specifications.table && data.topShapes) {
    ...
    record.frameMaterial = data.frameMaterial; paths.push("specifications.table", "frameMaterial");
  }
  ...
  if (record.specifications.outdoor && data.weatherResistanceClaim) {
    ...
    paths.push("specifications.outdoor", "referenceConfiguration", "dimensions");
  }
  if (record.specifications.carpet && data.composition) {
    ...
    record.materialComposition = data.composition; paths.push("specifications.carpet", "materialComposition", "dimensions");
  }
  if (record.specifications.lamp && data.luminousFluxLm !== undefined) {
    ...
    record.dimensions = record.variants[0].dimensions; paths.push("specifications.lamp", "variants", "dimensions");
  }
...
const important = {
  ...
  verifiedBatchFacts: (record) => record.dataQuality.verifiedFields.length > 0
};
