    ...
    {advice ? <div className="material-advice-results" aria-live="polite">
      ...
      {!hideMaterialCards ? recommended.map((material) => {
        ...
        return <article key={material.id}>
          ...
          <div><p className="stitch-eyebrow">Material ID {material.id}</p><h3>{material.name}</h3><p>Suitable because: {reasons.suitable.join("; ")}.</p><p><strong>Care effort:</strong> {material.easyCare === true ? "Lower based on authorized type guidance" : material.easyCare === false ? "Follow the recorded specialist guidance" : "Not verified for this illustrative profile"} · <strong>Durability:</strong> {material.durability == null ? "Not verified" : `${material.durability}/5`} · <strong>Light sensitivity:</strong> {material.lightSensitivity === "unknown" ? "Not verified" : material.lightSensitivity}</p>
