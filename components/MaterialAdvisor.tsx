...
import { Check, Lightbulb, Star, SwatchBook } from "lucide-react";
import React, { useEffect, useState } from "react";
import { materials, products } from "@/lib/data";
import { materialGuidance, materialReasons } from "@/lib/assistant";
...
export function MaterialAdvisor({ hideMaterialCards = false, onRecommendationsChange }: MaterialAdvisorProps) {
  ...
  const [answeredRequestText, setAnsweredRequestText] = useState("");
  ...
  const submit = async (question = requestText) => {
    const normalizedQuestion = question.trim();
    if (normalizedQuestion.length < 3) { setStatus("error"); return; }
    setStatus("loading"); setNotice(""); onRecommendationsChange?.(null);
    storage.track({ name: "material_advisor_submitted" });
    const response = await fetch("/api/ai/material-advice", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ requestText: normalizedQuestion }) }).catch(() => null);
    const payload = response ? await response.json().catch(() => null) : null;
    if (!response?.ok || !payload?.recommendedMaterialIds) { setStatus("error"); return; }
    setAdvice(payload); setAnsweredRequestText(normalizedQuestion); setStatus("idle"); setAnalysisSource(payload.ai?.mode === "openai" ? "openai" : payload.ai?.mode === "catalogue" ? "catalogue" : "demo"); setWasCached(Boolean(payload.ai?.cached));
    onRecommendationsChange?.({ materialIds: payload.recommendedMaterialIds, requestText: normalizedQuestion });
  };
  ...
  const recommended = advice ? advice.recommendedMaterialIds.map((id) => materials.find((material) => material.id === id)).filter((material): material is (typeof materials)[number] => Boolean(material)) : [];
  const avoided = advice ? advice.materialsToAvoid.map((id) => materials.find((material) => material.id === id)).filter((material): material is (typeof materials)[number] => Boolean(material)) : [];
  const guidance = advice ? materialGuidance(answeredRequestText, advice) : [];
  return <section className="material-advisor" aria-labelledby="material-advisor-title">
    <div><p className="stitch-eyebrow">AI Material & Care Advisor</p><h2 id="material-advisor-title"><SwatchBook aria-hidden="true" />Material & Care Advisor</h2><p>Swatch names and colours are illustrative until supplier records are connected. Performance is recommended only when supported by <a href="https://www.musterring.com/en/service/faq" target="_blank" rel="noreferrer">Musterring care guidance</a> or exact-cover tests following recognized <a href="https://contracttextiles.org/performance-guidelines/" target="_blank" rel="noreferrer">textile performance criteria</a>; missing evidence is shown as not verified.</p></div>
    <div className="material-advisor-input"><label htmlFor="material-advisor-request">Describe your home and everyday needs</label><textarea id="material-advisor-request" value={requestText} onChange={(event) => setRequestText(event.target.value)} /><button onClick={() => void submit()} disabled={status === "loading"}><Lightbulb aria-hidden="true" />{status === "loading" ? "Checking material metadata…" : "Advise me"}</button></div>
    ...
    {advice ? <div className="material-advice-results" aria-live="polite">
      <header><div className="material-advice-heading"><h3>{recommended.length ? "Recommended planning profiles" : "Material guidance from available data"}</h3><small className="material-analysis-source">{wasCached ? "Loaded from the material planning cache" : analysisSource === "openai" ? "Analysed with OpenAI using the material records below" : analysisSource === "catalogue" ? "Matched instantly against the recorded planning profiles" : "Catalogue fallback used"}. Suitability scores are illustrative until confirmed for the exact supplier cover.</small></div><div>{Object.entries(advice.needs).filter(([, value]) => value === true).map(([key]) => <span key={key}>{key.replace(/([A-Z])/g, " $1")}</span>)}</div></header>
      {guidance.length ? <div className="material-guidance-list">{guidance.map((item) => <article key={item.key}><Check aria-hidden="true" /><div><h4>{item.title}</h4><p>{item.answer}</p></div></article>)}</div> : null}
      {!hideMaterialCards ? recommended.map((material) => {
        ...
        return <article key={material.id}>
          ...
          <div><p className="stitch-eyebrow">Material ID {material.id}</p><h3>{material.name}</h3><p>Planning match because: {reasons.suitable.join("; ")}.</p><p><strong>Care effort:</strong> {material.easyCare === true ? "Lower-maintenance planning profile" : material.easyCare === false ? "More involved planning profile" : "Not verified for this illustrative profile"} · <strong>Durability:</strong> {material.durability == null ? "Not verified" : `${material.durability}/5 planning score`} · <strong>Light sensitivity:</strong> {material.lightSensitivity === "unknown" ? "Not verified" : `${material.lightSensitivity} planning profile`}</p>
