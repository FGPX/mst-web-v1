...
export default function InteriorStylistClient() {
  ...
  return <div className="stylist-page">
    ...
    {result ? <section className="stylist-results" id="stylist-results"><div className="container">
      <header className="stylist-result-head"><div><span className="stylist-kicker"><Sparkles size={15} /> Your grounded recommendations</span><h2>{result.title}</h2><p>{result.rationale}</p><div className={`stylist-match-note is-${result.catalogueMatch.level}`}><strong>{result.matchLevel === "exact" ? "Meets verified requirements" : result.recommendationMode === "set" ? "Closest coordinated set" : "Closest verified option"}</strong><span>{result.catalogueMatch.message}</span></div></div><div className="stylist-result-actions"><Link className="is-primary" href={roomComposerUploadHref(result.selections.map((selection) => selection.product.id))}><Armchair size={17} /> {result.roomType === "bathroom" ? "See this series in your room" : result.recommendationMode === "set" ? "See this set in your room" : "See this product in your room"}</Link><button type="button" onClick={saveSet} disabled={saved}><Save size={17} /> {saved ? "Saved to My Musterring" : result.selections.length === 1 ? "Save recommendation" : "Save complete set"}</button>{saved ? <Link href="/my-musterring">View saved set <ArrowRight size={16} /></Link> : null}</div></header>
