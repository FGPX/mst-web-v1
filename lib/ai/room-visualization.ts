...
function compositionInstructions(items: GroundedVisualizationItem[]) {
  ...
  if (hasCarpet) {
    guidance.push("The selected catalogue carpet is required and must remain visibly identifiable. Reproduce only its exact design from its numbered product reference: preserve the same pattern, motif geometry, colours, border, pile appearance, shape, and edge treatment. Use it to anchor the furniture group beneath the coffee table and at least the front legs of the main seating; do not omit it, hide it completely, redesign it, simplify its pattern, or place it remotely from the group.");
  }
  if (!hasCarpet) {
    guidance.push("No catalogue carpet was selected. Keep the visible floor uncovered: do not invent, retain, or add any rug, carpet, mat, or floor textile, even as a neutral styling accessory.");
  }
