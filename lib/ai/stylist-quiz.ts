...
export const stylistQuizByRoom: Record<StylistRoomType, StylistQuizQuestion[]> = {
  "living-room": [
    ...
    { id: "special-functions", prompt: "Do you want any special functions?", options: [option("relax-function", "Relax function"), option("recliner", "Recliner"), option("adjustable-headrest", "Adjustable headrest"), option("sofa-bed", "Sofa bed"), option("none", "No special functions")] },
  ...
  bedroom: [
    ...
    { id: "additional-storage", prompt: "Do you need additional storage?", options: [option("under-bed", "Under-bed storage"), option("wardrobe", "Wardrobe storage"), option("dresser", "Dresser storage"), option("no", "No")] },
    { id: "bed-priority", prompt: "What is most important for your bed?", options: [option("maximum-comfort", "Maximum comfort"), option("easy-access", "Easy access"), option("storage", "Storage"), option("design", "Design"), option("adjustable-functions", "Adjustable functions")] },
  ...
  hallway: [
    ...
    { id: "store-items", prompt: "What do you need to store?", options: [option("coats", "Coats"), option("shoes", "Shoes"), option("bags", "Bags"), option("accessories", "Accessories"), option("everything", "Everything")] },
  ...
  kitchen: [
    ...
    { id: "main-use", prompt: "How do you mainly use the space?", options: [option("cooking", "Cooking"), option("family-meals", "Family meals"), option("entertaining", "Entertaining"), option("quick-meals", "Quick meals"), option("everything", "Everything")] },
  ...
  outdoor: [
    ...
    { id: "main-use", prompt: "How will you mainly use the space?", options: [option("relaxing", "Relaxing"), option("dining", "Dining"), option("entertaining", "Entertaining"), option("sunbathing", "Sunbathing"), option("everything", "Everything")] },
    ...
    { id: "outdoor-priority", prompt: "What is most important?", options: [option("comfort", "Comfort"), option("easy-care", "Easy care"), option("weather-resistance", "Weather resistance"), option("flexible", "Flexible arrangement"), option("compact-size", "Compact size")] },
  ...
  "home-accessories": [
    ...
    { id: "goal", prompt: "What are you trying to achieve?", options: [option("cosier", "Make it cosier"), option("add-colour", "Add colour"), option("add-lighting", "Add lighting"), option("complete-room", "Complete the room"), option("refresh-style", "Refresh the style")] },
    { id: "existing-colours", prompt: "What colours are already in the room?", noteOption: "describe", noteLabel: "Describe the existing colours", options: [option("light-neutral", "Light neutrals"), option("warm-natural", "Warm natural tones"), option("dark", "Dark tones"), option("colourful", "Colourful"), option("describe", "I’ll describe them")] },
...
function firstMapped<T>(answers: Record<string, string>, map: Record<string, T>, fallback: T) {
  for (const value of Object.values(answers)) if (value in map) return map[value]!;
  return fallback;
}
...
export function validateStylistQuizInput(input: StylistQuizInput) {
  ...
  for (const question of questions) {
    ...
    if (!value || !question.options.some((candidate) => candidate.id === value)) return false;
    if (question.noteOption === value && !input.notes[question.id]?.trim()) return false;
  }
  const dimensionQuestion = questions.find((question) => question.dimensionOption && input.answers[question.id] === question.dimensionOption);
...
export function normalizeStylistQuiz(input: StylistQuizInput): StylistPreferences {
  ...
  const target = input.answers.target as StylistTarget;
  ...
  const style = firstMapped(aestheticAnswers, styleMap, "not-sure" as StylistStylePreference);
  const palette = firstMapped(input.answers, paletteMap, "no-preference" as StylistPalette);
  const materialValue = input.answers.material;
  ...
  const spaceValue = input.answers.space;
...
export function stylistAnswerLabel(roomType: StylistRoomType, questionId: string, answerId: string) {
  const question = stylistQuizByRoom[roomType].find((candidate) => candidate.id === questionId);
  return question?.options.find((candidate) => candidate.id === answerId)?.label ?? answerId.replaceAll("-", " ");
}
