...
export async function POST(request: Request) {
  ...
  const productIds = [...new Set(result.data.productIds)].filter((id) => activeProducts.has(id));
  const materialIds = [...new Set(result.data.materialIds)].filter((id) => validMaterials.has(id));
  ...
  const answer = {
    ...
    answer: lostProductGrounding
      ...
      : result.data.answer,
    answerType: lostProductGrounding ? "missing-data" as const : result.data.answerType,
    productIds,
    materialIds,
  ...
  return NextResponse.json({ answer, ai: { mode: result.provider, fallback: result.fallback } });
}
